    ...
    for (TRACKER_DATA& data : tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) {  //append
            ...
            if (newSize <= this->budget) {
                FEATURESS newSampleFeatures(newSize, Feature_Vector_Dim);
                newSampleFeatures.block(0, 0, oldSize, Feature_Vector_Dim) = samples[track_id];
                newSampleFeatures.block(oldSize, 0, addSize, Feature_Vector_Dim) = newFeatOne;
                samples[track_id] = newSampleFeatures;
            }
            else {
                if (oldSize < this->budget) {   //original space is not enough;
                    FEATURESS newSampleFeatures(this->budget, Feature_Vector_Dim);
                    if (addSize >= this->budget) {
                        newSampleFeatures = newFeatOne.block(0, 0, this->budget, Feature_Vector_Dim);
                    }
                    else {
                        newSampleFeatures.block(0, 0, this->budget - addSize, Feature_Vector_Dim) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, Feature_Vector_Dim).eval();
                        newSampleFeatures.block(this->budget - addSize, 0, addSize, Feature_Vector_Dim) = newFeatOne;
                    }
                    samples[track_id] = newSampleFeatures;
                }
                else {        //original space is ok;
                    if (addSize >= this->budget) {
                        samples[track_id] = newFeatOne.block(0, 0, this->budget, Feature_Vector_Dim);
                    }
                    else {
                        samples[track_id].block(0, 0, this->budget - addSize, Feature_Vector_Dim) =
                            samples[track_id].block(addSize - 1, 0, this->budget - addSize, Feature_Vector_Dim).eval();
                        samples[track_id].block(this->budget - addSize, 0, addSize, Feature_Vector_Dim) = newFeatOne;
                    }
                }
            }
        }
